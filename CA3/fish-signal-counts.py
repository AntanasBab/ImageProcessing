...
def analyze_images_with_smoothing(acridine_file, fitc_file, dapi_file):
    ...
    acridine_smoothed = apply_averaging_filter(acridine, kernel)
    fitc_smoothed = apply_averaging_filter(fitc, kernel)
    dapi_smoothed = apply_averaging_filter(dapi, kernel)
    
    # Create RGB image for visualization
    rgb_image = np.stack([acridine_smoothed, fitc_smoothed, dapi_smoothed], axis=-1)
    ...
    # Analyze each cell
    results = []
    acridine_threshold = otsu_threshold(acridine_smoothed)
    fitc_threshold = otsu_threshold(fitc_smoothed)
    
    for label in range(1, num_labels + 1):
        cell_mask = labels == label
        acridine_count = count_signals(cell_mask, acridine_smoothed, acridine_threshold)
        fitc_count = count_signals(cell_mask, fitc_smoothed, fitc_threshold)
...
acridine_file = "./ImgSetCa3/imgsetca3/BXY-ABCD_Region 002_FOV 00040_Acridine.tif"
dapi_file = "./ImgSetCa3/imgsetca3/BXY-ABCD_Region 002_FOV 00040_DAPI.tif"
fitc_file = "./ImgSetCa3/imgsetca3/BXY-ABCD_Region 002_FOV 00040_FITC.tif"

# Run the analysis
analyze_images_with_smoothing(acridine_file, fitc_file, dapi_file)
